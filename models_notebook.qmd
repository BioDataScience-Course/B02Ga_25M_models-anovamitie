...
Le principal point recherché est de savoir si la morphologie ou le pattern des jeunes serpents influe sur l'attaque du serpent adulte ou non. En effet, des serpents adultes de l'espèce *kingsnake* et de l'espèce *cornsnake* ont été mis en contact avec des jeunes de chaque espèce afin de voir combien de temps s'écoulait avant l'attaque, s'il y en avait une. Un autre point recherché est le lien entre la morphologie des juvéniles et le temps avant l'attaque.
...
-   Les juvéniles de l'espèce *kingsnake* ont tous une masse supérieure à celle de l'espèce *cornsnake*.
...
Le modèle réalisé n'est pas significatif et pertinent. Seule l'ordonnée à l'origine est significative au seuil alpha de 5%. De plus, le R² est proche de 0 ce qui indique que notre modèle n'est pas bien ajusté.
...
L'analyse du graphique composite des résidus nous indique que la normalité des résidus ne suit pas une distribution normale car les résidus s'écartent de manière symétrique aux extrémités. Les résidus sont élevés par rapport aux valeurs prédites. Les points semblent avoir une distance de Cook relativement élevée. L'homoscédasticité est respectée.

Ce test a été réalisé car nous pensions qu'il y aurait une relation entre le temps d'attaque et la relation entre la masse de l'adulte et du juvénile. Pour nous, les adultes de grande taille ciblent des juvéniles proportionnellement plus grands.
...
L'interprétation du tableau récapitulatif du modèle linéaire nous indique que le modèle est très ajusté avec un R\^2 = 0.98 et qu'il est significatif. (F-stat : 8735 on 1 and 198 df - p value : \<2.22e-16). L'ordonnée à l'origine est significativement différente de 0, par contre, elle est faiblement estimée avec un rapport de \~1/6 entre l'écart type et la valeur estimée. La pente est significativement différente de 0 et est estimée avec bonne précision.
...
-   Les effets de l’interaction entre le motif et l’espèce : L’effet du motif « Painted » sur le temps d’attaque est significativement différent au seuil alpha de 5% selon l’espèce de la proie. Lorsque la proie est peinte et est un kingsnake, elle est attaquée plus vite que dans le cas de référence.
...
-   De plus, le graphique de l'analyse des résidus n'a pas énormément de sens étant donné que l'on travaille avec des données qualitatives dans ce cas. Toutes les conditions ne sont pas respectées.
...
-   Le coefficient du ratio_mass est positif : un ratio de masse plus élevé semble favoriser la présence d’une attaque. De plus, la p-value est extrêmement faible, l’effet du ratio des masses sur la présence ou non d’une attaque est significatif au seuil alpha de 5%.
...
-   L’ajustement du modèle est relativement bon, mais il y a une légère sous-dispersion. L'estimation des paramètres est relativement bonne (ratio_mass : 0.229/1.31 = 0.17 ; ratio_svl : 0.357/-2.21 = -0.16 ).

#### Modèle linéaire généralisé et/ou mixte 2

```{r glm 2}
glm2_2 <- glm(data = snakerev, time_to_attack_s ~ pattern + prey_species, family = gaussian)
summary(glm2_2) |>tabularise()

```

$$
`r eq__(glm2_2, use_coefs = TRUE, origdata = snakerev)`
$$
...
L'estimation des paramètres est moyenne pour patternPainted (ratio écart type vs valeur estimée ≈ 0.56), bonne pour patternPainted random (ratio ≈ 0.24.) et pour patternPatternless (ratio ≈ 0.16), et très bonne pour prey_specieskingsnake (ratio ≈ 0.11)

La moyenne du temps avant l'attaque diminue pour patternPainted, patternPainted random, patternPatternless, ce qui indique une attaque rapide. Pour prey_specieskingsnake, la moyenne augmente ce qui indique une attaque plus lente.
...
Les modèles linéaires généralisés nous montrent que en effet, la morphologie du juvénile a un effet significatif sur le temps d'attaque des adultes. Les patterns semblent être également significatifs sur le temps d'attaque dans ces glm.

Les modèles linéaires simples et régressions linéaires, quant à eux, présentent des données contradictoires avec les glm: en effet, ils montrent que nos variables "morphologie" et "patterns" ne sont pas significatifs. Néanmoins, nous pouvons invalider ces modèles étant donné que les conditions pour les réaliser ne sont pas correctes.

Les deux modèles que nous gardons pour le report seront les deux modèles linéaires généralisés.
