...
author: "Louise Dubrecq, Sarah Delapierre, Martin Jadouille, Charlotte Desaeger"
format:
  html:
    code-fold: true
    code-tools: true
...
```{r import}
# Lecture des données
snake <- read("data/snake.rds")
# Tableau des premières et dernières lignes
tabularise$headtail(snake)
```
...
```{r chart1-LouiseDubrecq}
chart(data = snake200, ~ pattern %fill=% pred_attempt) +
...
chart(data = snake200,  ~ prey_species %col=% adult_sex ) +
  geom_bar(position = "dodge") +
...
```{r chart3}
chart(data = snake200, adult_mass_g ~ time_to_attack_s %col=% prey_species) +
  geom_point() +
  labs(x = "Temps avant l'attaque [s]", y = "Masse de l'adulte [g]", col = "Espèce")

chart(data=snake200, adult_svl_cm ~time_to_attack_s %col=% adult_sex) +
  geom_point() +
...
snake_corr <- correlation(sselect(snake, ratio_mass, ratio_svl, adult_mass_g, adult_svl_cm, juvenile_mass_g, juvenile_svl_cm, time_to_attack_s) )

plot(snake_corr)


snake_lm1 <- lm(data = snake, time_to_attack_s ~ ratio_mass + ratio_svl )
tabularise(summary_(snake_lm1))
```
