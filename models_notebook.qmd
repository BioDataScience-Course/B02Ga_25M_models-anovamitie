...
chart(data = snake, ~ pattern %fill=% pred_attempt) +
...
chart(data = snake,  ~ prey_species %col=% adult_sex ) +
  geom_bar(position = "dodge") +
...
```{r chart3}
chart(data = snake, adult_mass_g ~ time_to_attack_s %col=% prey_species) +
  geom_point() +
  labs(x = "Temps avant l'attaque [s]", y = "Masse de l'adulte [g]", col = "Espèce")

chart(data=snake, adult_svl_cm ~time_to_attack_s %col=% adult_sex) +
  geom_point() +
...
```{r reg linéaire 2}
snake_lm2 <- lm(data = snake, juvenile_mass_g ~ juvenile_svl_cm +  )
tabularise(summary_(snake_lm2))
```
